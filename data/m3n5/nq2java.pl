...
my $cur_ver_str = 'm3n5';
my $base_dpath = '/home/nagai/kyojo/schemaOrg35_201905';
...
foreach my $domain_uri (sort keys %$domain_uris) {
	...
	# [個別対応] v3.5で不整合？ v3.6で削除
	if (       $domain_uri eq 'http://schema.org/geospatiallyContains'
			|| $domain_uri eq 'http://schema.org/geospatiallyCoveredBy'
			|| $domain_uri eq 'http://schema.org/geospatiallyCovers'
			|| $domain_uri eq 'http://schema.org/geospatiallyCrosses'
			|| $domain_uri eq 'http://schema.org/geospatiallyDisjoint'
			|| $domain_uri eq 'http://schema.org/geospatiallyEquals'
			|| $domain_uri eq 'http://schema.org/geospatiallyIntersects'
			|| $domain_uri eq 'http://schema.org/geospatiallyOverlaps'
			|| $domain_uri eq 'http://schema.org/geospatiallyTouches'
			|| $domain_uri eq 'http://schema.org/geospatiallyWithin') {
		next;
	}
	...
	if ($domain_uri =~ /\/\/[a-z0-9\-\.]*schema\.org\/(\w+)$/ || $domain_uri =~ /\/\/kyojo\.org\/schemaSpl\/(\w+)$/) {
		...
		# [個別対応] 特殊な名称
		if ($this_name2 eq 'LaTeX' || $this_name2 eq 'ThreeDimensionalModel') {
	...
	foreach my $domain_uri (sort { $domain_uris->{$a}->{this_name} cmp $domain_uris->{$b}->{this_name} } keys %$type_domain_uris) {
		...
		if (exists $domain->{'http://schema.org/domainIncludes'}) {
			...
			foreach my $dom_inc (sort { if_undef ($domain_uris->{$a}->{this_name}, '') cmp if_undef ($domain_uris->{$b}->{this_name}, '') } keys %$dom_incs) {
				...
				# [個別対応]
				if ($extension eq 'bib' && $type_name eq 'Clazz'
...
sub camelize {
	...
	if ($word eq 'emissions_co2') {
		...
		return 'latex';
	} elsif ($word eq '3DModel' || $word eq '3dModel' || $word eq '3d_model') {
		return 'threeDimensionalModel';
	}
...
sub constantize {
	...
	# [個別対応]
	if ($word eq '3DModel' || $word eq '3dModel') {
...
sub constantize2 {
	...
	if ($word eq 'EmissionsCO2') {
		...
		return 'LATEX';
	} elsif ($word eq '3DModel' || $word eq '3dModel') {
		return 'THREE_DIMENSIONAL_MODEL';
	}
