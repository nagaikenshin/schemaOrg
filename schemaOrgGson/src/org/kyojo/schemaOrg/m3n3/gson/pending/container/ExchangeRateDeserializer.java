...
public class ExchangeRateDeserializer implements JsonDeserializer<ExchangeRate> {
	...
	@Override
	public ExchangeRate deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext context)
			throws JsonParseException {
		if(jsonElement.isJsonPrimitive()) {
			return new EXCHANGE_RATE(jsonElement.getAsString());
		}

		JsonObject jsonObject = jsonElement.getAsJsonObject();
		ExchangeRate obj = new EXCHANGE_RATE();
		HashMap<String, Field> fldMap = new HashMap<>();
		Field[] flds = EXCHANGE_RATE.class.getFields();
		for(Field fld : flds) {
