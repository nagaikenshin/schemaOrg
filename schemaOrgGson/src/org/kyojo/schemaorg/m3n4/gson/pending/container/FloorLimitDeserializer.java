...
import org.kyojo.schemaorg.m3n4.pending.impl.FLOOR_LIMIT;
import org.kyojo.schemaorg.m3n4.pending.Container.FloorLimit;

public class FloorLimitDeserializer implements JsonDeserializer<FloorLimit> {

	@Override
	public FloorLimit deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext context)
			throws JsonParseException {
		if(jsonElement.isJsonPrimitive()) {
			return new FLOOR_LIMIT(jsonElement.getAsString());
		}

		JsonObject jsonObject = jsonElement.getAsJsonObject();
		FloorLimit obj = new FLOOR_LIMIT();
		HashMap<String, Field> fldMap = new HashMap<>();
		Field[] flds = FLOOR_LIMIT.class.getFields();
		for(Field fld : flds) {
