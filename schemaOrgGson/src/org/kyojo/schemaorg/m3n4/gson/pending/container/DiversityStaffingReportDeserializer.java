...
public class DiversityStaffingReportDeserializer implements JsonDeserializer<DiversityStaffingReport> {

	@Override
	public DiversityStaffingReport deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext context)
			throws JsonParseException {
		if(jsonElement.isJsonPrimitive()) {
			return new DIVERSITY_STAFFING_REPORT(jsonElement.getAsString());
		}

		JsonObject jsonObject = jsonElement.getAsJsonObject();
		DiversityStaffingReport obj = new DIVERSITY_STAFFING_REPORT();
		HashMap<String, Field> fldMap = new HashMap<>();
		Field[] flds = DIVERSITY_STAFFING_REPORT.class.getFields();
		for(Field fld : flds) {
