...
public class ReceiveActionDeserializer implements JsonDeserializer<ReceiveAction> {

	@Override
	public ReceiveAction deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext context)
			throws JsonParseException {
		if(jsonElement.isJsonPrimitive()) {
			return new RECEIVE_ACTION(jsonElement.getAsString());
		}

		JsonObject jsonObject = jsonElement.getAsJsonObject();
		ReceiveAction obj = new RECEIVE_ACTION();
		HashMap<String, Field> fldMap = new HashMap<>();
		Field[] flds = RECEIVE_ACTION.class.getFields();
		for(Field fld : flds) {
