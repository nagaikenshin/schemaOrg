...
public class OfferCatalogDeserializer implements JsonDeserializer<OfferCatalog> {

	@Override
	public OfferCatalog deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext context)
			throws JsonParseException {
		if(jsonElement.isJsonPrimitive()) {
			return new OFFER_CATALOG(jsonElement.getAsString());
		}

		JsonObject jsonObject = jsonElement.getAsJsonObject();
		OfferCatalog obj = new OFFER_CATALOG();
		HashMap<String, Field> fldMap = new HashMap<>();
		Field[] flds = OFFER_CATALOG.class.getFields();
		for(Field fld : flds) {
