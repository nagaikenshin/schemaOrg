...
public class WPSideBarDeserializer implements JsonDeserializer<WPSideBar> {

	@Override
	public WPSideBar deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext context)
			throws JsonParseException {
		if(jsonElement.isJsonPrimitive()) {
			return new WP_SIDE_BAR(jsonElement.getAsString());
		}

		JsonObject jsonObject = jsonElement.getAsJsonObject();
		WPSideBar obj = new WP_SIDE_BAR();
		HashMap<String, Field> fldMap = new HashMap<>();
		Field[] flds = WP_SIDE_BAR.class.getFields();
		for(Field fld : flds) {
