...
public class ValidForDeserializer implements JsonDeserializer<ValidFor> {

	@Override
	public ValidFor deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext context)
			throws JsonParseException {
		if(jsonElement.isJsonPrimitive()) {
			return new VALID_FOR(jsonElement.getAsString());
		}

		JsonObject jsonObject = jsonElement.getAsJsonObject();
		ValidFor obj = new VALID_FOR();
		HashMap<String, Field> fldMap = new HashMap<>();
		Field[] flds = VALID_FOR.class.getFields();
		for(Field fld : flds) {
