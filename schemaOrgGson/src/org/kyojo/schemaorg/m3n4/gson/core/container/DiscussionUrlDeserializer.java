...
public class DiscussionUrlDeserializer implements JsonDeserializer<DiscussionUrl> {

	@Override
	public DiscussionUrl deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext context)
			throws JsonParseException {
		if(jsonElement.isJsonPrimitive()) {
			return new DISCUSSION_URL(jsonElement.getAsString());
		}

		JsonObject jsonObject = jsonElement.getAsJsonObject();
		DiscussionUrl obj = new DISCUSSION_URL();
		HashMap<String, Field> fldMap = new HashMap<>();
		Field[] flds = DISCUSSION_URL.class.getFields();
		for(Field fld : flds) {
