...
public class NoteDigitalDocumentDeserializer implements JsonDeserializer<NoteDigitalDocument> {

	@Override
	public NoteDigitalDocument deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext context)
			throws JsonParseException {
		if(jsonElement.isJsonPrimitive()) {
			return new NOTE_DIGITAL_DOCUMENT(jsonElement.getAsString());
		}

		JsonObject jsonObject = jsonElement.getAsJsonObject();
		NoteDigitalDocument obj = new NOTE_DIGITAL_DOCUMENT();
		HashMap<String, Field> fldMap = new HashMap<>();
		Field[] flds = NOTE_DIGITAL_DOCUMENT.class.getFields();
		for(Field fld : flds) {
